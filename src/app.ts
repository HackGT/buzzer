...
function scheduleCMS() {
  const cmsUrl =
    process.env.CMS_URL && !DEVELOPMENT
      ? process.env.CMS_URL
      : "https://keystone.dev.hack.gt/admin/api";
  fetch(cmsUrl, {
    method: "POST",
    headers: {
    ...
    .then((result: any) => {
      ...
      info.forEach((e: any) => {
        ...
        console.log(title, startTime, difference);
...
async function runSetup() {
  await Promise.all(
    Object.keys(mediaAPI).map(async pluginKey => {
